...
const normalize = data.reduce((accumulator, currentValue) => {
  ...
  //{1: {id: 1, name: "Ahmed"}, 2: {id: 2, name: "Zhang"}}
  //{1: {id: 1, name: "Ahmed"}, 2: {id: 2, name: "Zhang"}, 3: {id: 3, name: "Sophie"}}
  return accumulator;
}, {});
// {} {id: 1, name "Ahmed"}
// {1: {id: 1, name: "Ahmed"}}
...
//
//
// normalize: no shallow copy of object, so the object is only copied in reference not in value
...
const makeRequestPromiseChain2 = (url) => {
  httpRequestWithPromise(url)
    .then(
      // handle resolved Promise
      (result) => {
...
//
//
const makeRequestAsyncAwaitFinished = async (url) => {
  try {
    // Add code here to log the status
    const { status } = await httpRequestWithPromise(url);
    console.log(status);
  } catch (error) {
    // Add code here to log the status
    const { status } = error;
